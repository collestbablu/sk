...
<input type="text" name="prd" tabindex="1"  onkeyup="getdata()" onClick="getdata()"  id="prd" style="width:230px;"  placeholder=" Search Items..." tabindex="5" class="form-control" >
...
function getRackFun(id){
	...
	var pri=id;
	var main_loc=document.getElementById("main_loc_two").value;
	var loc=document.getElementById("location_two_id").value;
	var pri_id=document.getElementById("pri_id").value;
	getPalletQty1(main_loc,loc,pri_id);
	var xhttp = new XMLHttpRequest();
...
function rackOne(id){
	...
	var pri_id=document.getElementById("pri_id").value;
 	var xhttp = new XMLHttpRequest();
	xhttp.open("GET", "getRackOne?rack_one_id="+pri+"&main_loc="+main_loc+"&loc="+loc+"&pri_id="+pri_id, false);
	xhttp.send();
...
<tr class="gradeA">
<th><input class="btn btn-primary btn-sm" type="button" value="SAVE"   id="sv1" onclick="fsv(this)" ></th>
</tr>
</tbody>
</div>
</div>
...
function getPalletQty1(main_loc,loc,pri_id)
{

		var strURL="getPalletQty1?main_loc="+main_loc+"&loc="+loc+"&pri_id="+pri_id;
		var req = getXMLHTTP();
		if (req) {
			req.onreadystatechange = function() {
				if (req.readyState == 4) {
					if (req.status == 200) {
					//var price=mtr*ext_per;
						document.getElementById('qty_pallet1').innerHTML=req.responseText;
//alert(idm);
					} else {
