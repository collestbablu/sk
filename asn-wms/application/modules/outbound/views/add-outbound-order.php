...
function fsv(v)
{
	var inv_c=document.getElementById("invoice_num").value;
	
	inv_check(inv_c);
	
	var inv_f=inv_check(inv_c);
	
var rc=document.getElementById("rows").value;

if(rc!=0 && Number(inv_f)==0 )
{
...
// ####### ends ##############//
	    ...
	    var i = r.parentNode.parentNode.rowIndex;
     var cnf = confirm('Are You Sure..??? You Want To Delete Line No 1.'+(id));
if (cnf== true)
 {
 document.getElementById("invoice").deleteRow(i);
  slr();
...
?>
	...
	<div class="main-content">
		...
		<ol class="breadcrumb breadcrumb-2"> 
			...
			<li><a class="btn btn-success btn-sm" href="<?=base_url();?><?=base_url();?>outbound/manageOutbound">Manage Outbound</a></li> 
			
		</ol>
		<ol class="breadcrumb breadcrumb-2"> 
				<li><a href="<?=base_url();?>master/Item/dashboar"><i class="fa fa-home"></i>Dashboard</a></li> 
				<li><a href="#">Outbound</a></li> 
				<li><a href="#">Pre Outbound</a></li> 
				<li class="active"><strong><a href="#">Add Outbound</a></strong></li> 
			</ol>
		<?php }?>
...
<form class="form-horizontal" onSubmit="return checkKeyPressed(a)" method="post" action="insertStockOut" enctype="multipart/form-data">
...
<div class="col-sm-4">
 <select name="contact_id" id="contact_id"  onChange="getOrg(this.value);"  class="form-control ui fluid search dropdown email"  <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
	...
	?>
   <option value="<?=$getBooking->code;?>" <?php if($getBooking->contact_id==$branchFetch->contact_id){?> selected <?php }?>><?=$getBooking->first_name;?></option>
   <?php }?>
  </select>
...
<div class="col-sm-4" id="regid"> 
<input type="text" name="invoice_no" id="invoice_num"  value="<?php echo $branchFetch->invoice_no; ?>" <?php if($_GET['view']!='') {?> readonly="" required <?php }?> class="form-control" >
</div> 
</div>
...
<th><input type="text" readonly="" id="des1" style="width:90px;" class="form-control"> </th>
<th><input type="text" readonly="" id="des2" style="width:250px;" class="form-control"> </th>
...
<th>
<select  name="saleamnt" id="main_loc" class="form-control"  style="width:70px;"/ >
	<?php
...
<th>
<select name="saleamnt" id="loc" onChange="getPallet(this.value);" onFocus="getPallet(this.value);"  class="form-control"   style="width:70px;"/ >
	<?php
...
<th>
<select name="saleamnt11" id="rack_id" onChange="getrackQty1(this.value);" class="form-control"   style="width:70px;"/ >
	<?php
...
<th><input type="number" min="1"    style="width:70px;" id="qn"  class="form-control"> </th>
<th><input type="text" readonly="" id="qty_stock" style="width:70px;" class="form-control"> </th>
...
<th>&nbsp;</th>
<th><input class="btn btn-primary btn-sm" type="button" value="SAVE"   id="sv1" onClick="fsv(this)" >&nbsp;<a href="<?=base_url();?>outbound/manageOutbound" class="btn btn-blue btn-sm">Cancel</a></th></th>
</tr>
</tbody>
</table>
</div>
...
function inv_check(inv_c)
{
	
	
 var xhttp = new XMLHttpRequest();
  xhttp.open("GET", "inv_check?con="+inv_c, false);
  xhttp.send();
//alert(xhttp.responseText);
var inv_data=xhttp.responseText;

if(Number(inv_data)>0)
{
alert("Invoice number already exits");

}
//  document.getElementById("inv_check").innerHTML = xhttp.responseText;
return inv_data
