...
<?php
$z=1;
$query_dtl=$this->db->query("select SUM(quantity) asqtyQ,product_id,quantity from tbl_stock_out_dtl where stockouthdr='".$_GET['id']."' or stockouthdr='".$_GET['view']."' group by product_id ");
...
<input type="hidden" name="type[]" value="warehouse_transfer" />
...
$queryInbondQtyCnt=$this->db->query("select * from tbl_product_serial_log where product_id='$getProductName->Product_id' and inbound_id='".$_GET['id']."' and type='warehouse_transfer'");

$queryInbondQty=$this->db->query("select SUM(quantity) as Qtyy from tbl_product_serial_log where product_id='$getProductName->Product_id' and inbound_id='".$_GET['id']."' and type='warehouse_transfer' ");
...
<th>&nbsp;</th>
<th>&nbsp;</th>
<th><input class="btn btn-primary" name="save" id="bt"  type="submit" value="Stock In" /></th>
