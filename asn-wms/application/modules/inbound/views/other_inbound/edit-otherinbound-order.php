...
var rate=document.getElementById("lpr").innerHTML;
...
var entr =(e.keyCode);
if(document.getElementById("tot").value=="" && entr==08){
 ...
 function adda()
	...
	var cell=cell+indexcell;
        cell = row.insertCell(indexcell);
				...
				cell.align="center";
				//cell.style.display="none";
				var lprR = document.createElement("input");
							lprR.type="text";
							lprR.border ="0";
							lprR.value=lpr;	    
							lprR.name ='list_price[]';
							lprR.id='lpr'+rid;
							lprR.readOnly = true;
							lprR.style="text-align:center";
							lprR.style.width="100%";
							lprR.style.border="hidden"; 
							cell.appendChild(lprR);
...
<div class="col-sm-4">
  <select name="supplier_id" onChange="getOrg(this.value);"  class="form-control ui fluid search dropdown email"  <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
    <option value="">----Select ----</option>
    <?php
	$bookingType=$this->db->query("select *from tbl_contact_m  where group_name='4'");
	foreach($bookingType->result() as $getBooking){
	?>
   <option value="<?=$getBooking->code;?>" <?php if($getBooking->code==$branchFetch->supplier_id){?> selected <?php }?>><?=$getBooking->first_name;?></option>
   <?php }?>
  </select>
...
<select name="type"  class="form-control ui fluid search dropdown email"  <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
    <option value="">----Select ----</option>
    <?php
	$bookingType=$this->db->query("select *from tbl_master_data  where param_id='30'");
	foreach($bookingType->result() as $getBooking){
	?>
...
<div class="col-sm-4" id="regid"> 
<select name="origin"  id="div_con_id"   class="form-control ui fluid search dropdown email1"  <?php if(@$_GET['view']!=''){ ?> disabled="disabled" <?php }?> >
<?php
   $sqlgroup=$this->db->query("select * from tbl_contact_m where contact_id='$branchFetch->origin'");
...
<div class="form-group"> 
<label class="col-sm-2 control-label">Ref. NO./Challan no.:</label> 
<div class="col-sm-4" id="regid"> 
<input type="text" class="form-control" name="ref_no" value="<?php echo $branchFetch->ref_no; ?>">
  <input type="hidden" name="order_type" value="other_inbound">
</div>  
...
<th style="display:none;">International Tariff No.</th>
...
<th style="display:none;"><input type="number" step="any" id="lph" min="1"  value="" class="form-control" style="width:90px;">
</th>
...
<th style="display:none;"><input type="number" step="any" name="saleamnt" id="tariff_no" class="form-control"   style="width:90px;"/ ></th>

<th style="display:none;"><input type="number" step="any" name="saleamnt" id="doms_tariff_no" class="form-control"   style="width:90px;"/ ></th>
...
<td style="width:3%;display:none;"><div align="center"><u>Int.Tariff</u></div></td>
<td style="width:3%;display:none;"><div align="center"><u>Dom.Tariff</u></div></td>
...
<input type="text" name="list_price[]" id="lpr<?php echo $z;?>" value="<?php echo $invoiceFetch->list_price;?>" readonly="" style="text-align: center; width: 100%; border: hidden;">
...
<td align="center" style="width: 3%;display:none;"><input type="text" name="tariff_no[]" id="tariff_no<?php echo $z;?>" value="<?php echo $invoiceFetch->tariff_no;?>"readonly="" style="text-align: center; width: 100%; border: hidden;"></td>
...
<script>
function getOrg(supplier_id)
{
	
 var xhttp = new XMLHttpRequest();
  xhttp.open("GET", "getCity_fun?con="+supplier_id, false);
  xhttp.send();
