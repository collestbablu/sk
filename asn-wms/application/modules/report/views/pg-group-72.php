...
<a href="<?=base_url('report/Report/export_pg_group').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&filter='.$_GET['filter'];?>" class="dt-button buttons-excel buttons-html5" style="margin: 0px 30px 0px 0px;"><strong>Excel</strong></a>
</div>
</div>

<div class="dataTables_length" id="DataTables_Table_0_length" style="margin: 0px 0px 0px 30px;">
<label> Show
<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url('/report/Report/searchPgGroup72').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&filter='.$_GET['filter'];?>" class="form-control input-sm">
...
<tr>
		...
		<th><div style="width:100px;">Delhi</div></th>
		<th><div style="width:100px;">Bangalore</div></th>
		<th><div style="width:100px;">Del Pallet</div></th>
		<th><div style="width:100px;">Service</div></th>
		<th><div style="width:100px;">Sales Return</div></th>
...
<th><?php
$ttlblock=0;
$block=$this->db->query("select *,SUM(S.quantity) as sum_qty from tbl_product_serial S where S.product_id='".$rows->Product_id."' AND S.location_id='76' AND comp_id='".$this->session->userdata('comp_id')."' ")->row();
