...
<h4 class="panel-title">CONSUMER STOCK OF BLR REPORT </h4>
...
<a href="<?=base_url('report/Report/export_consumer_stock_blr').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&filter='.$_GET['filter'];?>" class="dt-button buttons-excel buttons-html5" style="margin: 0px 30px 0px 0px;"><strong>Excel</strong></a>
</div>
</div>

<div class="dataTables_length" id="DataTables_Table_0_length" style="margin: 0px 0px 0px 30px;">
<label> Show
<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url('/report/Report/searchConsumerBLRStock').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&filter='.$_GET['filter'];?>" class="form-control input-sm">
...
<thead>
<tr>
 		<th><div style="width:100px;">Serial No.</div></th>
		<th><div style="width:100px;">PG Group</div></th>
		<th><div style="width:100px;">Category</div></th>
		<th><div style="width:100px;">Model No.</div></th>
        <th><div style="width:110px;">Description 1</div></th>   
		<th><div style="width:110px;">Description 2</div></th>
		<th><div style="width:100px;">Block</div></th>
		<th style="display:none;"><div style="width:100px;">Delhi</div></th>
		<th><div style="width:100px;">Bangalore</div></th>
		<th style="display:none;"><div style="width:100px;">Del Pallet</div></th>
		<th style="display:none;"><div style="width:100px;">Service</div></th>
		<th style="display:none;"><div style="width:100px;">Sales Return</div></th>
...
if($blockB->sum_qty != '')
	echo $ttlblock = $blockB->sum_qty;
else
	echo "0";  ?>
</th>	

<th style="display:none;"><?php 
/*$delhi=$this->db->query("select SUM(quantity) as sum_qty from tbl_product_serial  where product_id='".$rows->Product_id."' AND location_id='54' ");
$sumDelhi=$delhi->row();*/
$ttldelhi=0;
...
<th><?php 
$ttlbangalore=0;
$bangalore=$this->db->query("select *,SUM(S.quantity) as sum_qty from tbl_product_serial S where S.product_id='".$rows->Product_id."' AND S.main_location_id='3' ")->row();
...
<th><?php 
//echo $total = $ttlblock + $ttldelhi + $ttlbangalore + $ttldellpallet + $ttlservice + $ttl_sales_return ;
echo $total = $ttlblock +  $ttlbangalore; ?>
</th>
