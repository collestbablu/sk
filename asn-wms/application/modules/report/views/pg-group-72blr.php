...
<h4 class="panel-title">PG GROUP 72 REPORT </h4>
...
<a href="<?=base_url('report/Report/export_pg_group_blr').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&filter='.$_GET['filter'];?>" class="dt-button buttons-excel buttons-html5" style="margin: 0px 30px 0px 0px;"><strong>Excel</strong></a>
</div>
</div>

<div class="dataTables_length" id="DataTables_Table_0_length" style="margin: 0px 0px 0px 30px;">
<label> Show
<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url('/report/Report/searchPgGroup72');?>" class="form-control input-sm">
...
<th><?php
$ttlblock=0;
$block=$this->db->query("select *,SUM(S.quantity) as sum_qty from tbl_product_serial S where S.product_id='".$rows->Product_id."' AND S.location_id='76' AND comp_id='3' ")->row();

if($block->sum_qty!='')
  echo $ttlblock = $block->sum_qty;  
else
  echo "0"; ?>
</th>	
...
<th><?php //echo $total = $ttlblock + $ttldelhi + $ttlbangalore + $ttldellpallet + $ttlservice + $ttl_sales_return ;
