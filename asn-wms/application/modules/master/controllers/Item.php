...
public function import_product_insert(){
 @extract($_POST);
 $filename=$_FILES["file"]["tmp_name"];
...
public function import_product_update()
{
	@extract($_POST);
	$filename=$_FILES["file"]["tmp_name"];
	if($_FILES["file"]["size"] > 0)
...
if($getData[0]!='Model')
{
	...
	$this->db->query("update  tbl_product_stock set sku_no='".$getData[0]."',productname='".addslashes($getData[1])."',category='".$catRow->prodcatg_id."',pg_group='".$getData[3]."',manufacturer_code='".$getData[4]."',inventoy_type='".$getData[5]."',hsn_code='".$getData[6]."',gst='".$getData[7]."',gst_tax='".$getData[7]."',family_group='".$getData[8]."',description2='".addslashes($getData[9])."',ean_number='".$getData[10]."',upc_code='".$getData[11]."',mrp='".$getData[12]."',length='".$getData[13]."',width='".$getData[14]."',height='".$getData[15]."',sales_pack_wt='".$getData[16]."',moq='".$getData[17]."',tariff='".$getData[18]."',comp_id='".$this->session->userdata('comp_id')."' where sku_no='".$getData[0]."'");
...
public function import_data_outbound_hdr()
{
...
$filename=$_FILES["file"]["tmp_name"];
		...
		if($_FILES["file"]["size"] > 0)
		{
	        ...
	        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
	        {
				...
				//echo $getData[0];
				//$productQuery=$this->db->query("select *from tbl_contact_m where first_name='".$getData[7]."'");
				//$getProduct=$productQuery->row();
				//$getProduct->Product_id;

				//$location=$this->db->query("select * from tbl_contact_m where city='".$getData[8]."' ");
				//$getLoc=$location->row();
			 
		 
		 
				
				$this->db->query("insert into tbl_return_hdr set rgp_gate_pass_date='$getData[0]',return_due_date='$getData[1]',rgp_gatepass_no='$getData[2]',manual_gate_pass_no='$getData[3]',manual_gp_date='$getData[4]',gatepass_issued_name='$getData[5]',responsibility='$getData[6]',stock_position='$getData[7]',location_to='$getData[6]',wh_remarks='$getData[9]',remarks1='$getData[10]',remarks2='$getData[11]'");			   
...
public function import_data_outbound_dtl()
{
...
$filename=$_FILES["file"]["tmp_name"];
		...
		if($_FILES["file"]["size"] > 0)
		{
	        ...
	        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
	        {
				...
				//echo $getData[0];die;
				$rthdr=$this->db->query("select *from tbl_return_hdr where manual_gate_pass_no='".$getData[0]."'");
				$getHdr=$rthdr->row();

				$pid=$this->db->query("select * from tbl_product_stock where sku_no='".$getData[1]."' ");
				$getPid=$pid->row();

				$loc=$this->db->query("select * from tbl_master_data where keyvalue='".$getData[3]."' ");
				$getLoc=$loc->row();
			 
		 
		 
				
				$this->db->query("insert into tbl_return_dtl set returnhdr='$getHdr->returnid',product_id='$getPid->Product_id',main_loc='$getData[2]',loc='$getLoc->serial_number',rack_id='$getData[4]',quantity='$getData[5]'");			   
