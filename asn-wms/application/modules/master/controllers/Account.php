...
public function export_contact()
{
	...
	{
		$this->load->view('/Account/export-contact-list');
	}
	else
	{
		redirect('index');
...
function Manage_Contact_Join()
{
      ...
      if($_GET['entries']!="" && $_GET['filter']!='filter')
	   ...
	   {
	     $url = site_url('/master/Account/manage_contact?code='.$_GET['code'].'&name='.$_GET['name'].'&loc='.$_GET['loc'].'&group='.$_GET['group'].'&contact='.$_GET['contact'].'&email='.$_GET['email'].'&mobile='.$_GET['mobile'].'&phone='.$_GET['phone'].'&filter='.$_GET['filter'].'&entries='.$_GET['entries']);
		}
	   else
	    {
...
public function import_contact()
{
	if($this->session->userdata('is_logged_in'))
	{
		$this->load->view('Account/import-contact');
	}
	else
	{
		redirect('index');
...
public function import_contact_insert()
{
	...
	if($getData[0]!='id')	
	  {
		$this->db->query("insert into tbl_contact_m set contact_id='".$getData[0]."',first_name='".$getData[1]."',country='".$getData[2]."',pin_code='".$getData[3]."',city='".$getData[4]."',address1='".addslashes($getData[5])."',address3='".addslashes($getData[6])."',address4='".addslashes($getData[7])."',address5='".addslashes($getData[8])."',code='".$getData[9]."',gst='".$getData[10]."',IT_Pan='".$getData[11]."',comp_id='".$this->session->userdata('comp_id')."'");
